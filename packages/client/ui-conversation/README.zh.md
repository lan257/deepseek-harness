...
已结算轮次会折叠其整个工作过程，transcript 中只留下最终回答：轮次中途的 assistant 叙述（进度正文与推理）完全隐藏，工具调用过程则在首个调用的位置折叠为一行展开项——`已执行 N 个工具调用`——点击该行可展开或再次收起卡片；完整过程仍可在轨迹视图查看。折叠以时间线（turn/end）中的轮次关闭状态为准，绝不依据 step 状态：结算的工具 step 会在轮次中途关闭而 agent 仍在工作，只有轮次结束才代表过程完成。轮次打开期间运行中的 step 保持可见；已关闭轮次中未结算的调用仍保留其卡片；不带轮次坐标的 session 作用域节点不受影响。逐轮展开标记放在共享 chat store（`expandedProcessTurns` 与 `toggleProcessExpanded`）中，因此展开状态在切换视图标签和刷新后仍保留；未展开的轮次保持折叠。
